...
contains
  subroutine mlw1dsrc1st(u,f,um,umm,du,s,ix,dt,dx)
    ...
    double precision,intent(in) :: u(ix),f(ix),s(ix)
    double precision,intent(in) :: dt,dx
    double precision,intent(inout) :: um(ix),umm(ix),du(ix)
    !!
    !! um : t+dt,n+1/2; umm : t+dt, n
    !! du : contribute to next dt
    !!
    integer :: i
    ...
    do i = 1, ix-1
    if(i>1) then
      du(i) =  - 0.5d0*dt/dx * 0.5d0*(f(i+1)-f(i-1)) + 0.5d0*dt*s(i)
    endif
  ...
  subroutine mlw1dsrc2nd(fm,du,smm,ix,dt,dx)
    ...
    do i = 2, ix-1
    du(i) = -0.5d0*dt/dx*(fm(i)-fm(i-1)) + 0.5d0*dt*smm(i)
    enddo
