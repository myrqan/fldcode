...
for i in range(num_steps):
    tle = r"$t=$"+ str(tt[i])[:5]
    ax.plot(xx,uu[i],label=tle,linestyle='-',color=colors[i])
    ax.plot(xx,qq[i],marker='+',markevery=m_sp,markersize=4,linestyle='-',color=colors[i])
    ax2.plot(xx,(uu[i]-qq[i]),label=tle)
#print(xx)

plt.tight_layout()
plt.legend()
plt.savefig(f"gx{np.size(xx)}.png",dpi=300)
#plt.show()
